...
library(- - -) # to build decision trees
library(- - -)  # to visualise decision trees
library(- - -)  # for simple scatter plot visualisations
library(- - -)  # to test models and make predictions
library(- - -)   # evaluating classification performance using confusion matrix
```
...
```{r}
data_labeled <-  - - -[!is.na(- - -),]# This is the way we could have our dataset without NA's for the variable "decision"
...
sample <- sample.int(n = - - -(- - -), size = floor(.75*nrow(- - -)), replace = - - -)
