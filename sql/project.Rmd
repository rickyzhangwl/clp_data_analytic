...
This article is to analyze the sales data of a model kit provider. Their product line consists of model ships, planes, cars and military ground vehicles. I will access the data by creating connection to the database.

As the sales data including the information of customers, orders, products, productlines and the company employees, I will answer **below business problems** in this article, so that the management could better understand the company's business status.

1. How is the sales of each productline?
...
I will use advanced sql funcions such as subquery, OLAP, window function to answer.
...
We see an outlier! By checking the hovertext, we can know it is **1992 Ferrari 360 Spider Red**. It is so popular as only this classic car has an ordered quantity over 1800, while all of other products are sold with a number between 767 and 1111. Besides, we don't have a product with very low ordered quantity.

By the filter, we can see that the **Train** productline only has 3 products. This is the contributing factor to its low sales.
...
FROM supSub_rz
```
That's it! The company has 22 employees in total, 1 President, 1 VP Sales. 3 Regional Sales Manger and 1 Sales Rep directly report to the VP Sales. The four guys also have their subordinates reporting to them.
